    ...
    while True:
        ...
        if pose_results.keypoints is not None:
            ...
            for person_kpts in keypoints:
                ...
                person_center_y = valid_points[:, 1].mean()

                if PLAYER_SIDE == "far" and person_center_y > split_line:
                    continue
                elif PLAYER_SIDE == "near" and person_center_y < split_line:
                    continue
                ...
                if len(wrists) > 0:
                    current_wrist_center = sum(wrists) / len(wrists)

                # Draw selected player keypoints
        ...
        if current_wrist_center is not None and prev_wrist_center is not None:
            movement = ((current_wrist_center[0] - prev_wrist_center[0]) ** 2 +
                        (current_wrist_center[1] - prev_wrist_center[1]) ** 2) ** 0.5
